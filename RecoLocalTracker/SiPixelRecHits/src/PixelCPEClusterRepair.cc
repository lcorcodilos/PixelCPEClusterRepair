...
: PixelCPEBase(conf, mag, geom, ttopo, lorentzAngle, 0, templateDBobject, 0,1)
{
   ...
   if ( LoadTemplatesFromDB_ )
   {
      ...
      if ( !SiPixelTemplate2D::pushfile( *templateDBobject_, thePixelTemp_) )
   ...
   {
      //cout << "PixelCPEClusterRepair : Loading templates 40 and 41 from ASCII files ------------------------" << endl;
      
      if ( !SiPixelTemplate2D::pushfile( 40, thePixelTemp_ ) )
         throw cms::Exception("PixelCPEClusterRepair")
         << "\nERROR: Templates 40 not loaded correctly from text file. Reconstruction will fail.\n\n";
      
      if ( !SiPixelTemplate2D::pushfile( 41, thePixelTemp_ ) )
         throw cms::Exception("PixelCPEClusterRepair")
...
{
   return new ClusterParamTemplate2D(cl);
}
...
PixelCPEClusterRepair::localPosition(DetParam const & theDetParam, ClusterParam & theClusterParamBase) const
{
   ...
   ClusterParamTemplate2D & theClusterParam = static_cast<ClusterParamTemplate2D &>(theClusterParamBase);
   ...
   SiPixelTemplateReco2D::ClusMatrix clusterPayload{&clustMatrix[0][0], xdouble, ydouble, mrow,mcol};
   ...
   theClusterParam.templProbXY_ = theClusterParam.templProbQ_ = 1.0f;
   ...
   //  edgeflag - (input) flag to indicate the present of edges in y: 0-none, 1-edge at
   //    deltay - (output) template y-length - cluster length [when > 0, possibly missing end]

   int edgeflag = 0;  // ???? what to put here?
   float deltay = 0;
   ...
   {
      cout << " PixelCPEClusterRepair : We should never be here !!!!!!!!!!!!!!!!!!!!!!" << endl;
   ...
   else // apparenly this is he good one!
   {
      ...
      if ( DoLorentz_ ) {
         ...
         if( theDetParam.lorentzShiftInCmX!= 0.0 ||  theDetParam.lorentzShiftInCmY!= 0.0 ) {
            ...
            //&&& check with Morris: float templateLorbiasCmX = -micronsToCm*templ.lorxbias();  // new
            //&&& check with Morris: float templateLorbiasCmY = -micronsToCm*templ.lorybias(); //incl. 1/2
            ...
            //&&& check with Morris: theClusterParam.templXrec_ += (0.5*(theDetParam.lorentzShiftInCmX) - templateLorbiasCmX);
            //&&& check with Morris: theClusterParam.templYrec_ += (0.5*(theDetParam.lorentzShiftInCmY) - templateLorbiasCmY);
   ...
   //&&& For now store 2D xy prob in both probX and probY
   theClusterParam.probabilityX_  = theClusterParam.templProbXY_;
   theClusterParam.probabilityY_  = theClusterParam.templProbXY_;
...
PixelCPEClusterRepair::localError(DetParam const & theDetParam,  ClusterParam & theClusterParamBase) const
{
   ...
   else
   {
      ...
      else if ( edgex || edgey )
      {
         ...
         {
            throw cms::Exception(" PixelCPEClusterRepair::localError: Something wrong with pixel edge flag !!!");
         }
